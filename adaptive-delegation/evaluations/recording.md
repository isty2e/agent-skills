...
- Direct work remains valid; only actual launch attempts require observations.
...
- One bounded exposed lookup may fill missing fields; unknown, unavailable, failed, unattributable, and skipped collection
  are not zero.
- Parent verification, repair, disposition, and next choice update the same observation even when no hint changes.
- Missing authority or a failed write is disclosed without relocation or weaker acceptance.
- Concurrent edits preserve unrelated content; consolidation retains IDs, denominators, failures, missingness, and
  contrary evidence.
...
3. Run an ordinary delegated task. Compare runtime IDs with the pending record, statistics lookup, parent assessment, and
   persisted result. Reuse existing failures or cancellations rather than manufacturing costly ones.
4. In a fresh session, check that applicable hints and unfinished observations are read and reconciled without relaunching
   work to recreate metrics.
5. Report activation, collection, persistence, and assessment separately as observed, not observed, or not exercised.
   Claim complete capture only when an authoritative runtime inventory supports it.
